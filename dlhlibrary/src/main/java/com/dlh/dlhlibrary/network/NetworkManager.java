...
import com.dlh.dlhlibrary.network.errorhandler.ExceptionHandle;
import com.dlh.dlhlibrary.network.errorhandler.HttpErrorHandler;
import com.dlh.dlhlibrary.network.interceptor.RequestInterceptor;
import com.dlh.dlhlibrary.network.interceptor.ResponseInterceptor;
import com.dlh.dlhlibrary.utils.GsonTypeAdapterFactory;
...
public class NetworkManager {
    ...
    public static <T> T createService(Class<T> serviceClass, String url) {
        return getRetrofit(serviceClass,url).create(serviceClass);
    }
...
//            int cacheSize = 100 * 1024 * 1024;
     ...
     * @return Retrofit
     */
    private static Retrofit getRetrofit(Class serviceClass,String url) {
        if (retrofitHashMap.get(url + serviceClass.getName()) != null) {
            //刚才上面定义的Map中键是String，值是Retrofit，当键不为空时，必然有值，有值则直接返回。
            return retrofitHashMap.get(url + serviceClass.getName());
        }
        ...
        //放入Map中
        retrofitHashMap.put(url + serviceClass.getName(), retrofit);
        //最后返回即可
        return retrofit;
    }
    ...
    public static <T> ObservableTransformer<T, T> applySchedulers(final Observer<T> observer) {
        return upstream -> {
            Observable<T> observable = upstream
                    ...
                    .map(NetworkManager.getAppErrorHandler())//判断有没有500的错误，有则进入getAppErrorHandler
        ...
        return response -> {
            //当response返回出现500之类的错误时
              if (response instanceof BaseResponse) {
                  if(((BaseResponse) response).responseCode >= 500) {
